<?php
namespace Konafets\DoctrineDbal\Install\Service\Legacy;
 ...
 */
class SqlSchemaMigrationService extends \Konafets\DoctrineDbal\Install\Service\SqlSchemaMigrationService {
				...
				if ($value[0] === ')' && substr($value, -1) === ';') {
					...
					if (preg_match('/(COLLATE)[ ]*=[ ]*([a-zA-z0-9_-]+)/', $value, $tcollation)) {
						...
						if (preg_match('/(CHARSET|CHARACTER SET)[ ]*=[ ]*([a-zA-z0-9_-]+)/', $value, $tcharset)) {
							...
							$charset = $this->getDatabaseConnection()->getDatabaseCharset();
						}
	...
	public function getCollationForCharset($charset) {
		...
		if (!count($this->character_sets)) {
			if (method_exists($this->getDatabaseConnection(), 'listDatabaseCharsets')) {
				$this->character_sets = $this->getDatabaseConnection()->listDatabaseCharsets();
			} else {
	...
	public function getFieldDefinitions_database() {
		...
		$this->getDatabaseConnection()->sql_select_db();
		echo $this->getDatabaseConnection()->sql_error();
		$tables = $this->getDatabaseConnection()->listTables();
		foreach ($tables as $tableName => $tableStatus) {
			// Fields
			$fieldInformation = $this->getDatabaseConnection()->listFields($tableName);
			foreach ($fieldInformation as $fN => $fieldRow) {
				$total[$tableName]['fields'][$fN] = $this->assembleFieldDefinition($fieldRow);
			}
			// Keys
			$keyInformation = $this->getDatabaseConnection()->listKeys($tableName);
			...
			if (is_array($diffArr[$theKey])) {
				foreach ($diffArr[$theKey] as $table => $info) {
					...
					if (is_array($info['fields'])) {
						foreach ($info['fields'] as $fieldName => $fieldValue) {
							if ($info['whole_table']) {
								$whole_table[] = $fieldName . ' ' . $fieldValue;
							} else {
								// Special case to work around MySQL problems when adding auto_increment fields:
								if (stristr($fieldValue, 'auto_increment')) {
									...
									if (isset($diffArr['extra'][$table]['keys']['PRIMARY'])) {
										...
										$fieldValue = str_replace(' auto_increment', '', $fieldValue);
									} else {
								...
								if ($theKey == 'extra') {
									if ($remove) {
										if (substr($fieldName, 0, $deletedPrefixLength) !== $deletedPrefixKey) {
											// we've to make sure we don't exceed the maximal length
											$prefixedFieldName = $deletedPrefixKey . substr($fieldName, ($deletedPrefixLength - self::MYSQL_MAXIMUM_FIELD_WIDTH));
											$statement = 'ALTER TABLE ' . $table . ' CHANGE ' . $fieldName . ' ' . $prefixedFieldName . ' ' . $fieldValue . ';';
											$statements['change'][md5($statement)] = $statement;
										} else {
											$statement = 'ALTER TABLE ' . $table . ' DROP ' . $fieldName . ';';
											$statements['drop'][md5($statement)] = $statement;
										}
									} else {
										$statement = 'ALTER TABLE ' . $table . ' ADD ' . $fieldName . ' ' . $fieldValue . ';';
										$statements['add'][md5($statement)] = $statement;
									}
								} elseif ($theKey == 'diff') {
									$statement = 'ALTER TABLE ' . $table . ' CHANGE ' . $fieldName . ' ' . $fieldName . ' ' . $fieldValue . ';';
									$statements['change'][md5($statement)] = $statement;
									$statements['change_currentValue'][md5($statement)] = $diffArr['diff_currentValues'][$table]['fields'][$fieldName];
								}
							}
						}
					}
					if (is_array($info['keys'])) {
						foreach ($info['keys'] as $fieldName => $fieldValue) {
							if ($info['whole_table']) {
								$whole_table[] = $fieldValue;
							} else {
								if ($theKey == 'extra') {
									if ($remove) {
										$statement = 'ALTER TABLE ' . $table . ($fieldName == 'PRIMARY' ? ' DROP PRIMARY KEY' : ' DROP KEY ' . $fieldName) . ';';
										$statements['drop'][md5($statement)] = $statement;
									} else {
										$statement = 'ALTER TABLE ' . $table . ' ADD ' . $fieldValue . ';';
										$statements['add'][md5($statement)] = $statement;
									}
								} elseif ($theKey == 'diff') {
									$statement = 'ALTER TABLE ' . $table . ($fieldName == 'PRIMARY' ? ' DROP PRIMARY KEY' : ' DROP KEY ' . $fieldName) . ';';
					...
					if (is_array($info['extra'])) {
						...
						$clear_table = FALSE;
						foreach ($info['extra'] as $fieldName => $fieldValue) {
							...
							if (!$remove) {
								if (!$info['whole_table']) {
									...
									if ($fieldName == 'CLEAR') {
										...
										if (count($info['keys']) || $fieldValue == 2) {
											$clear_table = TRUE;
										}
										continue;
									} else {
										$extras[] = $fieldName . '=' . $fieldValue;
										$extras_currentValue[] = $fieldName . '=' . $diffArr['diff_currentValues'][$table]['extra'][$fieldName];
									}
								}
							}
						}
...
', $whole_table) . '
	 ...
	 */
	public function performUpdateQueries($arr, $keyArr = array()) {
		$result = array();
		if (ExtensionManagementUtility::isLoaded('doctrine_dbal')) {
			$statements = $arr;
			if (is_array($statements)) {
				foreach ($statements as $key => $string) {
					$res = $this->getDatabaseConnection()->adminQuery($string);
					if ($res === FALSE) {
						$result[$key] = $this->getDatabaseConnection()->sql_error();
					} elseif (is_resource($res) || is_a($res, '\\mysqli_result')) {
						$this->getDatabaseConnection()->freeResult($res);
					}
				}
			}
		} else {
